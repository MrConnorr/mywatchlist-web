...
function Comments(props)
{
    ...
    useEffect(() =>
    {
        fetch(`https://mywatchlistapi.onrender.com/comments/${mediaType}/${watchObjId}`)
                    ...
                    {comments.slice(0, secondSlice).map(comment =>
                        <div key={comment.id} className={styles.comment_container} style={{marginLeft: comment.replyToReply ? "75px" : comment.replyTo ? "37.5px" : null, borderLeft: comment.replyTo || comment.replyToReply ? "2px dashed" : null}}>
                            <div className={styles.comment_content}>
                                <img src={`${comment.deleted ? `https://mywatchlistapi.onrender.com/usersProfilePics/default.jpg` : `https://res.cloudinary.com/hlzbzu7fj/image/upload/${comment.createdBy.username}-profilePic.png`}`} alt={`${comment.createdBy.username} profile picture`} />
